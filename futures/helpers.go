/*
FILE: futures/helpers.go

DESCRIPTION:
Shared helpers for the KuCoin Futures sub-clients: canonical validation
error constructor, client-order-id generation, and small REST invocation
wrappers that decode the KuCoin envelope's data field into a typed value.
*/
...
// kept local so stream/book code reads cleanly.
...
func errInvalidRequest(method, msg string) error {
	return kucoin.NewError(kucoin.ErrorKindInvalidRequest, "", "futures."+method+": "+msg, nil)
}

// errAuthRequired is returned by signed calls when the client has no
// credentials configured.
func errAuthRequired(method string) error {
	return kucoin.NewError(kucoin.ErrorKindAuth, "", "futures."+method+": signed endpoint requires API credentials", nil)
}
...
// the transport, and decodes the data field. Signed calls short-circuit
// with errAuthRequired when no credentials are configured so callers get a
// typed auth error instead of a server round-trip.
...
// nsToMs converts a nanosecond timestamp to milliseconds. KuCoin ships
// ticker / trade / execution timestamps in nanoseconds.
...
// toValues converts a flat string map into url.Values. Kept tiny and local
// so call sites read as plain maps.
