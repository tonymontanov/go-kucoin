...
DESCRIPTION:
Public WebSocket sub-client for the KuCoin Futures profile. Wires the
...
no auth). The private connection (see stream-private.go) uses the signed
private bullet. Both are created lazily on first use and survive reconnects
transparently (the transport resubscribes every registered topic).

TOPICS (public):
  - /contractMarket/level2:{symbol}      incremental order book (managed)
  - /contractMarket/ticker:{symbol}      real-time ticker (last + best bid/ask)
  - /contractMarket/execution:{symbol}   public trade tape
  - /contractMarket/candle:{symbol}_{g}  klines

HANDLER CONTRACT (HOT PATH):
Handlers run synchronously on the read-loop goroutine. They MUST be O(1)
and non-blocking — copy what you need and return. A blocking handler stalls
every topic on the same socket.

ORDER BOOK:
KuCoin Futures level2 is a SEQUENCE-based incremental stream with no pushed
snapshot. WatchOrderBook seeds a local sequence engine from the REST
snapshot, applies the change stream, detects gaps and re-seeds (re-fetch
snapshot) automatically, emitting a consistent OrderBookSnapshot per update.
*/
...
// The handler receives a consistent snapshot after every applied change;
// the SDK seeds from REST, reconciles by sequence and re-seeds on a gap.
...
func (s *StreamClient) WatchTicker(ctx context.Context, symbol string, handler func(*futurestypes.MarketTicker)) error {
	if symbol == "" || handler == nil {
	...
	return conn.Subscribe(&ws.Subscription{
		...
		Handler: func(_, _ string, data []byte) {
			...
			var t futurestypes.MarketTicker = w.toTicker(symbol)
			handler(&t)
		},
	})
}
...
func (s *StreamClient) WatchKlines(ctx context.Context, symbol string, tf futurestypes.Timeframe, handler func(*roottypes.KlineUpdate)) error {
	if symbol == "" || handler == nil {
	...
	return conn.Subscribe(&ws.Subscription{
		Topic:   "/contractMarket/candle:" + symbol + "_" + gran,
		Handler: km.onMessage,
		Reset:   km.onReset,
	})
}

// WatchInstrument subscribes to the mark/index price stream for a symbol
// (topic "/contract/instrument:{symbol}", subject "mark.index.price"). The
// same topic also carries a "funding.rate" subject; this watcher filters it
// out and only delivers mark/index updates. KuCoin's ticker channel does NOT
// carry mark/index price, so this is the dedicated low-latency source for
// both values.
func (s *StreamClient) WatchInstrument(ctx context.Context, symbol string, handler func(*futurestypes.MarkPrice)) error {
	if symbol == "" || handler == nil {
		return errInvalidRequest("WatchInstrument", "symbol and handler are required")
	}
	var conn *ws.Conn = s.ensurePublic(ctx)
	return conn.Subscribe(&ws.Subscription{
		Topic: "/contract/instrument:" + symbol,
		Handler: func(_, subject string, data []byte) {
			if subject != "mark.index.price" {
				return
			}
			var w instrumentPushWire
			if codecUnmarshal(data, &w) != nil {
				return
			}
			var mp futurestypes.MarkPrice
			var ok bool
			mp, ok = w.toMarkPrice(symbol)
			if !ok {
				return
			}
			handler(&mp)
...
// pendingChange is a level2 change buffered while the engine re-seeds.
...
// onMessage applies one level2 change frame.
func (b *bookManager) onMessage(_, _ string, data []byte) {
	...
	var err error
	side, price, size, err = orderbook.ParseChange(w.Change)
	if err != nil {
		return
	}
	...
	b.mu.Lock()
	if !b.seeded {
		if len(b.pending) < pendingChangeCap {
			b.pending = append(b.pending, pendingChange{side: side, price: price, size: size, seq: w.Sequence, ts: w.Timestamp})
		}
		b.mu.Unlock()
		return
	}
	err = b.engine.ApplyChange(side, price, size, w.Sequence, w.Timestamp)
...
func (b *bookManager) reseed() {
	...
	for i = 0; i < len(b.pending); i++ {
		...
		_ = b.engine.ApplyChange(p.side, p.price, p.size, p.seq, p.ts)
	}
	b.pending = b.pending[:0]
