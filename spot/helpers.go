/*
FILE: spot/helpers.go

DESCRIPTION:
Shared helpers for the KuCoin Spot sub-clients: validation/auth error
constructors, client-order-id generation, REST invocation wrappers that
decode the KuCoin envelope's data field, the spot kline-granularity mapping
...
func errInvalidRequest(method, msg string) error {
	return kucoin.NewError(kucoin.ErrorKindInvalidRequest, "", "spot."+method+": "+msg, nil)
}

// errAuthRequired is returned by signed calls when the client has no
// credentials configured.
func errAuthRequired(method string) error {
	return kucoin.NewError(kucoin.ErrorKindAuth, "", "spot."+method+": signed endpoint requires API credentials", nil)
}
...
func generateClientOid() string {
	return "kc-" + strconv.FormatInt(time.Now().UnixNano(), 10) + "-" +
...
// nsToMs converts a nanosecond timestamp to milliseconds. KuCoin Spot ships
// trade-tape (match / histories) timestamps in nanoseconds; ticker / level1
// already arrive in milliseconds.
...
// spotGranularity maps the layer-1 Timeframe onto KuCoin Spot's kline `type`
// string ("1min"/"1hour"/"1day"/…). Empty for an unsupported timeframe so
// callers can validate before sending. NB: this differs from the futures
// granularity, which is an integer minute count.
var spotGranularity = map[spottypes.Timeframe]string{
	spottypes.Timeframe1m:  "1min",
	spottypes.Timeframe5m:  "5min",
	spottypes.Timeframe15m: "15min",
	spottypes.Timeframe30m: "30min",
	spottypes.Timeframe1h:  "1hour",
	spottypes.Timeframe2h:  "2hour",
	spottypes.Timeframe4h:  "4hour",
	spottypes.Timeframe8h:  "8hour",
	spottypes.Timeframe12h: "12hour",
